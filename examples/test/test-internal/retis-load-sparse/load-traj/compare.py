...
"""Result comparison for RETIS load-traj simulation.
...
Determinism is guaranteed by:
  - Fixed seeds in retis.rst (engine seed=0, TIS seed=1, velocity seed=0)
  - Initial paths loaded from committed trajectory files in pippo/
    (no randomness in path initialisation)

Three checks are performed:
  1. pathensemble.txt and order.txt match stored reference files.
...
def compare_files(settings):
    """Compare pathensemble.txt and order.txt for every ensemble.
...
def check_path_file(ens):
    ...
    ----------
    ens : object
        PathEnsemble object (already created from settings).
    ...
    valid_ends = ('R',) if ens.ensemble_number == 0 else ('R', 'L')
    broken = False
    ...
    with open(filename, 'r', encoding='utf-8') as fh:
        for line in fh:
            ...
            if right not in valid_ends:
                ...
                broken = True
            # Both relevant interfaces must lie strictly between mino and maxo
...
def _order_swap_ok(data0, data1, special):
    ...
    special == 0  : [0-] was generated from [0+]
                    last two frames of 0- == first two frames of 0+
    special == 1  : [0+] was generated from [0-]
                    first two frames of 0+ == last two frames of 0-
    otherwise     : full array match (numpy.allclose)
    """
    if special == 0:
...
def check_ensemble_swaps(settings):
    ...
    # Build path tables for all ensembles
    path_info = {}
    path_acc = {}
    for i in range(n):
        path_info[i], path_acc[i] = _read_path_table(i)

    # Annotate swap parents
    for i in range(n):
    ...
    status = 0
    for i in range(n):
        if i < n - 1:   # check s+ (current sends to right neighbour)
            special = 0 if i == 0 else 2
            status += _check_swaps_one_direction(
                path_info[i], path_acc[i], i, i + 1, 's+', special
            )
        if i > 0:       # check s- (current receives from left neighbour)
