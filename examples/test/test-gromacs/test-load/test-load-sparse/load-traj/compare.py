...
"""Result comparison for GROMACS RETIS load-traj simulation.

We compare a RETIS simulation of 5 steps — with initial paths loaded
from a gromacs trajectory file — against stored reference results.
...
To generate reference results, run the test once with GROMACS available:
    ./run.sh
    mkdir -p results/000 results/001 results/002
    for d in 000 001 002; do
