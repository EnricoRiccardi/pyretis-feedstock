...
def sparse_load_check_output_files(settings):
    """Verify that sparse-load output files exist and are non-empty."""
...
def sparse_load_check_path_file(ens, valid_ends=('R', 'L')):
    ...
    with open(filename, 'r', encoding='utf-8') as fh:
        for line in fh:
            ...
            if left != start:
                logger.error(
                    'Wrong start at step %d: %s != %s', step, left, start
                )
                broken = True
            if middle != 'M':
...
def _order_swap_ok(data0, data1, special):
    ...
    if special == 0:
        return (
            np.isclose(data0[-2][1], data1[0][1])
            and np.isclose(data0[-1][1], data1[1][1])
        )
    if special == 1:
        return (
            np.isclose(data0[0][1], data1[-2][1])
            and np.isclose(data0[1][1], data1[-1][1])
        )
...
def _check_swaps_one_direction(
    paths, path_acc, ens_number, partner_number, move_symbol, special
):
    ...
    for traj0 in traj0_list:
        ...
        if traj1 is None:
            logger.warning(
                'Parent %d not found for swap at step %d', parent_idx, idx0
            )
            ok = False
            continue
