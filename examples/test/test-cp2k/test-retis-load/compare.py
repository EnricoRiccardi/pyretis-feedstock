...
"""Result comparison for CP2K RETIS load simulation.
...
def compare_files(settings):
    ...
    # CP2K tests use a slightly looser tolerance
    rel_tol = 1e-4
        ...
        for file_name in files:
            ...
            if file_name == 'pathensemble.txt':
                equal, msg = compare_path_ensemble_data(
                    result_new, result_old, rel_tol=rel_tol
                )
            else:
                equal, msg = compare_numerical_data(
                    result_new, result_old, rel_tol=rel_tol
                )
